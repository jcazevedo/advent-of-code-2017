...
object Day20 extends App with AdventOfCode {
  val particles: Seq[(Particle, Int)] = loadFile(20).map { line =>
  ...
  def go2(particles: List[Particle], runs: Int = 10000): Int = {
    ...
    runs match {
      ...
      case other => go2(particlesWithoutCollisions.map(_.next), runs - 1)
    }
  }

  println(go1(particles.toList))
  println(go2(particles.toList.map(_._1)))
}
